...
This document describes the experimental design as it was planned: the motivation, the operational definition of the "step 3" construct being probed for, the six-step procedure, how the contrastive prompt dataset was constructed, and the concerns that were flagged *before* running anything. It reflects design-time decisions, not results.

See [RESULTS.md](RESULTS.md) for what actually happened when this plan was executed, including how several of the concerns below played out in practice - some were resolved, at least one was not, and dataset pair 9 surfaced a distinction (genuine randomness vs. deterministic mimicry of the same computational shape) that this document doesn't spell out on its own.

## Motivation

Executing low-probability action is a key part of game-theoretic strategies designed to drive up testing costs (Redwood Research's AI-control framing: Greenblatt, Shlegeris, Sachan & Roger, ["AI Control: Improving Safety Despite Intentional Subversion,"](https://arxiv.org/abs/2312.06942) ICML 2024, arXiv:2312.06942). Considering and executing low-probability action can occur through some really predictable mechanisms (though other mechanisms haven't been ruled out) (Serrano, Xing, Lindner & Jenner, ["Frontier Models Can Take Actions at Low Probabilities,"](https://arxiv.org/abs/2603.02202) arXiv:2603.02202, accepted ICLR 2026). Even if those consideration mechanisms do appear in other contexts, they are bayesian evidence in favor of attempting a rare-defection strategy.

This is specifically about *probabilistic* rarity, not rarity in general. A periodic or otherwise deterministic rare-trigger (e.g. "act on exactly every 1000th opportunity") is fully predictable in advance and is already cheaply caught by ordinary pattern detection on the action log - it can't serve the evasion function that makes rare defection a threat in the first place. Genuine calibrated randomness is the load-bearing property: it's what makes the specific triggering instance unpredictable to an overseer, which is what makes internal (activation-level) detection, rather than external log analysis, the relevant tool.
...
## Acknowledged Issues (Design-Time)

These were flagged as risks before running the experiment. See RESULTS.md for how each actually played out.
